...
function getDigitalRoot(num) {
  const sum = num.toString().split('').map((item) => +item).reduce((acc, item) => acc + item, 0);
  if (sum > 9) {
    return sum.toString().split('').map((item) => +item).reduce((acc, item) => acc + item, 0);
  }
  return sum;
}
